...
public class BasykParser 
{
	...
	private void Clear()
	{
		...
		Parse("{=collatzVar del { collatzVar / 2 } { collatzVar * 3 + 1 } (collatzVar % 2 == 0) ifelse ~collatzVar } =collatz pop", null);
		Parse("{=duplicator $duplicator ~duplicator} =dup pop", null);
		Parse("{=col; { col } { col collatz =col pop; col != 1 } while; col; ~col } =CollatzSimulation pop", null);
		...
		for(String tok : tokens)
		{
			...
			if(tok.equals("parse") || tok.equals("exe"))
			{
				if(stack.GetSize() != 0)
				{
					...
					{
						//then do something
						Parse(((StackFunction)stack.Pop()).GetFunction(), true, player);
